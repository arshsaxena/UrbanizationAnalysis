...
\usepackage{xcolor}
\usepackage{listings}
\usepackage{inconsolata} % Added for proper monospace font
\usepackage{array}
\usepackage{tabularx}
\usepackage{caption} % Added to support \caption* used in the figures
\usepackage{url} % Added to properly format the GitHub URL

% Table formatting definitions for vertical and horizontal centering
\renewcommand{\tabularxcolumn}[1]{m{#1}} % Vertically center X columns
\newcolumntype{C}{>{\centering\arraybackslash}X} % Horizontally center X columns
...
\lstset{
  ...
  commentstyle=\color{green!60!black}\itshape\ttfamily,
  numbers=left,
  ...
  escapeinside={(*}{*)},
  morekeywords={var,ee,Map,print}
}

\begin{document}

\title{Satellite-Based Urban Growth Analysis Using Sentinel-1/2 Data}
...
\begin{IEEEkeywords}
Urban Growth, Remote Sensing, Sentinel-1, Sentinel-2, NDBI, SAR, Chennai, Multi-temporal Analysis
\end{IEEEkeywords}
...
\begin{itemize}
    \item \textbf{Latitude Range:} $12.80^\circ - 13.20^\circ$ North
    \item \textbf{Longitude Range:} $80.10^\circ - 80.40^\circ$ East
    \item \textbf{Total Area:} Approximately 1,200 km$^2$
    \item \textbf{Administrative Boundaries:} Chennai Metropolitan Area (CMA)
\end{itemize}
...
\caption{Satellite Data Specifications and Applications}
\begin{center}
\begin{tabularx}{\columnwidth}{|C|C|C|C|}
\hline
\textbf{Satellite} & \textbf{Sensor Type} & \textbf{Resolution} & \textbf{Application} \\
\hline
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{../assets/block_diagram.png}
\caption{Methodological workflow for multi-sensor urbanization analysis using optical (Sentinel-2) and microwave (Sentinel-1) remote sensing data.}
\label{fig:block_diagram}
\end{figure}

\subsection{Optical Remote Sensing Analysis}
...
\subsubsection{NDBI Computation and Theoretical Foundation}
...
\subsubsection{Classification and Area Calculation}
...
\subsection{Microwave Remote Sensing Analysis}
...
\subsubsection{Backscatter Analysis Principles}
...
\subsubsection{Urban Detection Threshold}
...
\subsection{Optical Analysis Results}
...
\begin{itemize}
    \item \textbf{Urban Area 2018:} 158.96 km$^2$
    \item \textbf{Urban Area 2024:} 166.39 km$^2$
    \item \textbf{Net Urban Expansion:} 7.43 km$^2$ (4.7\% increase)
    \item \textbf{Annual Growth Rate:} 1.24 km$^2$/year
\end{itemize}
...
\centering
\includegraphics[width=\linewidth]{../data/gee/urban_area2018.png}
\caption*{(a) 2018 NDBI Urban Detection}
\end{minipage}
\hfill
\begin{minipage}{0.48\columnwidth}
\centering
\includegraphics[width=\linewidth]{../data/gee/urban_area2024.png}
\caption*{(b) 2024 NDBI Urban Detection}
\end{minipage}
\caption{Sentinel-2 NDBI urban area detection comparison (2018-2024). Red pixels represent built-up urban land detected through normalized difference built-up index analysis.}
\label{fig:ndbi_comparison}
\end{figure}

\subsection{Radar Analysis Results}
...
\begin{itemize}
    \item \textbf{Radar Urban Area 2018:} 592.43 km$^2$
    \item \textbf{Radar Urban Area 2024:} 680.58 km$^2$
    \item \textbf{Net Urban Expansion:} 88.15 km$^2$ (14.9\% increase)
    \item \textbf{Annual Growth Rate:} 14.69 km$^2$/year
\end{itemize}
...
\begin{minipage}{0.48\columnwidth}
\centering
\includegraphics[width=\linewidth]{../data/gee/radar_image2018.png}
\caption*{(a) 2018 SAR Backscatter}
\end{minipage}
\hfill
\begin{minipage}{0.48\columnwidth}
\centering
\includegraphics[width=\linewidth]{../data/gee/radar_image2024.png}
\caption*{(b) 2024 SAR Backscatter}
\end{minipage}
\caption{Sentinel-1 VV polarization backscatter intensity comparison (2018-2024). Images show raw radar backscatter data revealing surface roughness and structural characteristics across Chennai metropolitan area.}
\label{fig:radar_backscatter_comparison}
\end{figure}
...
\begin{minipage}{0.48\columnwidth}
\centering
\includegraphics[width=\linewidth]{../data/gee/radar_urban2018.png}
\caption*{(a) SAR Classification 2018}
\end{minipage}
\hfill
\begin{minipage}{0.48\columnwidth}
\centering
\includegraphics[width=\linewidth]{../data/gee/radar_urban2024.png}
\caption*{(b) SAR Classification 2024}
\end{minipage}
\caption{Sentinel-1 SAR urban classification comparison (2018-2024). Blue pixels show radar-detected urban areas using VV $>$ -10 dB threshold.}
\label{fig:sar_classification_comparison}
\end{figure}
...
\subsection{Sensor-Specific Urban Detection Characteristics}
...
\subsection{Urban Growth Pattern Analysis}
...
\subsubsection{Baseline Population Data}
Official Indian Census records provide the foundation for demographic analysis:
\begin{itemize}
    \item \textbf{2001 Census Population:} 43,43,645
    \item \textbf{2021 Census Population:} 46,46,732
\end{itemize}

\subsubsection{Growth Rate Calculation}
Total population growth over the 20-year census period:
\begin{align}
\text{Total Growth} &= 46,46,732 - 43,43,645 \nonumber \\ 
&= 3,03,087 \text{ people}
\end{align}

Average annual population growth rate:
\begin{align}
\text{Annual Growth} &= \frac{3,03,087}{20} \nonumber \\
&= 15,154 \text{ people/year}
\end{align}

\subsubsection{Study Period Population Estimates}
\textbf{2018 Population Estimation:}
\begin{align}
\text{Years post-2001} &= 2018 - 2001 = 17 \text{ yrs}\\
\text{Pop. increase} &= 17 \times 15,154 = 2,57,618\\
\text{Est. 2018 pop.} &= 43,43,645 + 2,57,618 \nonumber \\
&= 46,01,263
\end{align}

\textbf{2024 Population Estimation:}
\begin{align}
\text{Years post-2021} &= 2024 - 2021 = 3 \text{ yrs}\\
\text{Pop. increase} &= 3 \times 15,154 = 45,462\\
\text{Est. 2024 pop.} &= 46,46,732 + 45,462 \nonumber \\
&= 46,92,194
\end{align}

\begin{table}[htbp]
\renewcommand{\arraystretch}{1.3} % Adds vertical padding to rows
\caption{Chennai Population Growth Analysis (2001-2024)}
\begin{center}
\begin{tabularx}{\columnwidth}{|C|C|C|}
\hline
\textbf{Year} & \textbf{Population} & \textbf{Data Source} \\
\hline
...
\hline
\end{tabularx}
\label{tab_population_data}
\end{center}
\end{table}

\subsubsection{Validation of Spatial Expansion}
The demographic analysis reveals substantial population pressure during the study period (2018-2024):
\begin{itemize}
    \item \textbf{Study Period Population Growth:} 90,931 people (46,92,194 - 46,01,263)
    \item \textbf{Annual Demographic Pressure:} 15,154 people/year requiring urban accommodation
    \item \textbf{Population Growth Rate:} 1.98\% over 6 years (0.33\% annually)
\end{itemize}

This population growth supports the urban expansion observed in the satellite analysis. The relationship between population increase and spatial growth shows that both the optical results (7.43 km$^2$, 4.7\% increase) and radar results (88.15 km$^2$, 14.9\% increase) represent real urban development and are not caused by measurement errors. The population growth rate (1.98\% over 6 years) is close to the moderate growth detected by optical data (4.7\%), which suggests that the city is growing both through increased density and expansion of built-up areas.

\section{Design Code Implementation}
...
\begin{itemize}
    \item Optical NDBI Detection: Sentinel-2 multispectral analysis using the normalized difference built-up index
    \item Microwave SAR Detection: Sentinel-1 radar backscatter analysis to detect urban surfaces
    \item Combined Analysis: An integrated method that uses both datasets for more complete urban monitoring
\end{itemize}

All code files include comments and explanations describing the methodology and expected results. The programs can be run directly in the Google Earth Engine Code Editor, so the analysis can be reproduced. The complete open-source code is available at: \url{https://github.com/arshsaxena/UrbanizationAnalysis}
...
\bibitem{b1} 
European Space Agency (ESA), ``Sentinel-1 User Guide,'' [Online]. Available: https://sentinel.esa.int/web/sentinel/user-guides/sentinel-1-sar

\bibitem{b2} 
European Space Agency (ESA), ``Sentinel-2 MSI User Handbook,'' [Online]. Available: https://sentinel.esa.int/web/sentinel/user-guides/sentinel-2-msi

\bibitem{b3} 
Google Earth Engine, ``Google Earth Engine Data Catalog,'' [Online]. Available: https://developers.google.com/earth-engine/datasets

\bibitem{b4} 
Office of the Registrar General \& Census Commissioner, India, ``Census of India – Population Statistics,'' Ministry of Home Affairs, Government of India.
